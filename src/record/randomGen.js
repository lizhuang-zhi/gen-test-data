...
import { RandomGenFieldIDs, FieldType } from "@/record/field.js";
...
*/
export function textTimeFieldRandomAdd(timeStrVal, addTime) {
...
*/
export function randomTimeRecord(record, fieldIdTypeMap) {
    ...
    for (let field_id in fields) {
        if (RandomGenFieldIDs.includes(field_id)) {
            ...
            if (fieldIdTypeMap[field_id] === FieldType.TEXT) {
                resultRecord.fields[field_id] = [
                    {
                        text: textTimeFieldRandomAdd(fields[field_id][0].text, randomNum + randomUnit),
                        type: "text"
                    }
                ]
            } 
            // else if(fieldIdTypeMap[field_id] === FieldType.TIME) {
            //     resultRecord.fields[field_id] = 
            // }
